...
def capture_feedback(db: Session, email_id: int, old_label: str, corrected_label: str,
                     corrected_by: Optional[str] = None, note: Optional[str] = None) -> FeedbackRecord:
    fb_repo = FeedbackRepository(db)
    audit = AuditLogRepository(db)

    fb = FeedbackRecord(
        email_id=email_id, old_label=old_label,
    ...
    audit.log(email_id=email_id, event_type="feedback_captured",
